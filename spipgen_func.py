...
@jit(nopython=True)
def ucold_tab(rho, material, ucold_table):
    """
...
@jit(nopython=True)
def find_rho(Ps, material, K, alpha, rho0, rho1, ucold_table):
    """
    ...
    f0 = Ps - P_EoS((ucold_tab(rho0, material, ucold_table) + c*T_of_rho(rho0, K, alpha)), rho0, material)
    f1 = Ps - P_EoS((ucold_tab(rho1, material, ucold_table) + c*T_of_rho(rho1, K, alpha)), rho1, material)
    ...
    if f0*f1 > 0:
        ...
        while np.abs(rho1 - rho0) > tolerance:
            f0 = Ps - P_EoS((ucold_tab(rho0, material, ucold_table) + c*T_of_rho(rho0, K, alpha)), rho0, material)
            f1 = Ps - P_EoS((ucold_tab(rho1, material, ucold_table) + c*T_of_rho(rho1, K, alpha)), rho1, material)
            f2 = Ps - P_EoS((ucold_tab(rho2, material, ucold_table) + c*T_of_rho(rho2, K, alpha)), rho2, material)
...
@jit(nopython=True)
def Vg(rc, z, rho_grid, Rs, I_array):
    """
    ...
    for i in range(rho_grid.shape[0] - int(N/2)):
        ...
        for j in range(rho_grid.shape[1]):
            ...
            Vsum = Vsum + rho_grid[i,j]*rcp*I_tab(gamma, I_array)/np.sqrt(a2 + err)/np.sqrt(alpha)
...
@jit(nopython=True)
def fillV(rho_grid, Rs, Tw, I_array):
    """
    ...
    for i in range(V_grid.shape[0]):
        for j in range(V_grid.shape[1]):
            V_grid[i,j] = Vg(i*dr, j*dr, rho_grid, Rs, I_array) - (1/2)*(W*i*dr*R_earth)**2
            
    return V_grid

"""########################## Not sure if to include: first try of the function
# Fill rho grid given the potential
# attempt 1, from center to surface
@jit(nopython=True)
def fillrho(V_grid, Rs, rho_c, rho_s, material, K, alpha, ucold_table):
    ...
    for j in range(rho_grid.shape[1]):
        if j == 0:
            ...
            rho_prev = rho_grid[0, j - 1]
            P = (V_grid[0, j - 1] - V_grid[0, j])*rho_prev
            P = P + P_rho(rho_prev, material, K, alpha)
            if P > Ps:
                rho_grid[0,j] = find_rho(P, material, K, alpha, rho_s - 10, rho_prev, ucold_table)
            else:
                rho_grid[0,j] = 0
                break
        else:
            rho_prev = rho_grid[0, j - 1]
            P = (V_grid[0, j - 1] - V_grid[0, j + 1])*rho_prev/2
            P = P + P_rho(rho_prev, material, K, alpha)
            if P > Ps:
                rho_grid[0,j] = find_rho(P, material, K, alpha, rho_s - 10, rho_prev, ucold_table)
            else:
    ...
    # fill the rest
    for j in range(rho_grid.shape[1]):
        for i in range(1, rho_grid.shape[0]):
            rho_prev = rho_grid[i - 1, j]
            if rho_prev == 0:
                break
            P = (V_grid[i - 1, j] - V_grid[i + 1, j])*rho_prev/2
            P = P + P_rho(rho_prev, material, K, alpha)
            if P >= Ps:
                rho_grid[i,j] = find_rho(P, material, K, alpha, rho_s - 10, rho_prev, ucold_table)
            else:
                rho_grid[i,j] = 0
                break
...
@jit(nopython=True)
def fillrho(V_grid, Rs, prev_rho_grid, rho_s, material, K, alpha, ucold_table):
    """
    ...
    for i in range(2):
        for j in range(1, rho_grid.shape[1] - 1):
            ...
            if P >= Ps:
                rho_grid[i, j + 1] = find_rho(P, material, K, alpha, rho_s - 10, rho_grid[i,j], ucold_table)
            else:
                rho_grid[i, j + 1] = 0
                break       
    ...
    for j in range(0, rho_grid.shape[1]):
        for i in range(1, rho_grid.shape[0] - 1):
            ...
            if P >= Ps:
                rho_grid[i + 1, j] = find_rho(P, material, K, alpha, rho_s - 10, rho_grid[i,j], ucold_table)
            else:
                rho_grid[i + 1, j] = 0
                break
...
@jit(nopython=True)
def fillV_rows(rho_grid, Rs, I_array, lower, upper, rank, subsize, N, W, dr):
    """
    ...
    for i in range(2*N):
        for j in range(upper - lower):
            rows[i, j] = Vg(i*dr, (rank*subsize + j)*dr, rho_grid, Rs, I_array) - (1/2)*(W*i*dr*R_earth)**2
    
    return rows

def fillV_par(rho_grid, Rs, Tw, I_array):
    """
    ...
    V_subgrid = fillV_rows(rho_grid, Rs, I_array, lower, upper, rank, subsize, N, W, dr)
...
def main(iterations, rho_sph, Rs, material, Tw, K, alpha):
    ...
    N = rho_sph.shape[0]
    rho_s = rho_sph[-1]
    I_array = create_I_tab()
    ucold_table = create_ucold_table()
    ...
    for i in range(1, iterations + 1):
        ...
        start = time.time()
        V1 = fillV_par(rho[i - 1], Rs, Tw, I_array)
        end = time.time()
        times[i - 1, 0] = end - start
        start = time.time()
        rho[i] = fillrho(V1, Rs, rho[i - 1], rho_s, material, K, alpha, ucold_table)
        end = time.time()
