...
def _fillrho3(r_array, V_e, z_array, V_p, P_c, P_cm, P_ma, P_s, rho_c, rho_s,
    ...
    for i in range(r_array.shape[0] - 1):
        ...
        if P_e[i + 1] >= P_s and P_e[i + 1] >= P_cm:
            ...
            rho_e[i + 1] = _find_rho(P_e[i + 1], mat_id_atm, T_rho_id_atm, T_rho_args_atm,
    ...
    for i in range(z_array.shape[0] - 1):
        ...
        if P_p[i + 1] >= P_s and P_p[i + 1] >= P_cm:
            ...
            rho_p[i + 1] = _find_rho(P_p[i + 1], mat_id_atm, T_rho_id_atm, T_rho_args_atm,
...
def spin3layer(iterations, r_array, z_array, radii, densities, Tw,
    ...
    if mat_id_atm == 100:
        ucold_array_atm = np.load('ucold_array_100.npy')
    elif mat_id_atm == 101:
        ucold_array_atm = np.load('ucold_array_101.npy')
    elif mat_id_atm == 102:
        ucold_array_atm = np.load('ucold_array_102.npy')
...
def _picle_placement_3layer(r_array, rho_e, z_array, rho_p, Tw, N, rho_cm, rho_ma,
    ...
    for k in range(mP.shape[0]):
        if particles_rho[k] > rho_cm:
            ...
            u[k] = _ucold_tab(particles_rho[k], ucold_array_mantle)
            u[k] = u[k] + c_mantle*T_rho(particles_rho[k], T_rho_id_mantle, T_rho_args_mantle)
            
        else:
            u[k] = _ucold_tab(rho[k], ucold_array_atm)
            u[k] = u[k] + c_atm*T_rho(particles_rho[k], T_rho_id_atm, T_rho_args_atm)
...
data = pd.read_csv("3layer.csv", header=0)
...
densities = np.array(data.rho)
Tw = 4.
P_c = np.median(np.sort(data.P)[-500:])
P_cm = (199280358858.423 + 199376352169.541)/2.
P_ma = (20607549637.2677 + 20635273728.3554)/2.
P_s = np.min(data.P)
rho_c = np.median(np.sort(data.rho)[-500:])
rho_s = np.min(data.rho)
mat_id_core = 100
T_rho_id_core = 1
T_rho_args_core = [300, 0]
...
profile_e, profile_p = spin3layer(iterations, r_array, z_array, radii, densities, Tw,
                                  P_c, P_cm, P_ma, P_s, rho_c, rho_s,
                                  mat_id_core, T_rho_id_core, T_rho_args_core,
                                  mat_id_mantle, T_rho_id_mantle, T_rho_args_mantle,
                                  mat_id_atm, T_rho_id_atm, T_rho_args_atm)
...
x, y, z, vx, vy, vz, m, h, rho, P, u, picle_id, mat_id =                      \
_picle_placement_3layer(r_array, rho_e, z_array, rho_p, Tw, N, rho_cm, rho_ma,
                        mat_id_core, T_rho_id_core, T_rho_args_core,
                        mat_id_mantle, T_rho_id_mantle, T_rho_args_mantle,
                        mat_id_atm, T_rho_id_atm, T_rho_args_atm)
