...
swift_to_SI = Conversions(gv.M_earth, gv.R_earth, 1) # normal units for planetary impacts
cgs_to_SI   = Conversions(1e-3, 1e-2, 1)
SI_to_cgs   = cgs_to_SI.inv()

def save_picle_data(f, A2_pos, A2_vel, A1_m, A1_h, A1_rho, A1_P, A1_u, A1_id,
                    A1_mat_id, boxsize, file_to_SI, verbosity=1):
    ...
    if verbosity >= 1:
        ...
        if np.amax(abs((A2_pos + boxsize/2.)*SI_to_file.l)) > boxsize:
    ...
    grp.create_dataset(Di_hdf5_picle_label['pos'], data=(A2_pos + boxsize/2.)*SI_to_file.l, dtype='d')
    grp.create_dataset(Di_hdf5_picle_label['vel'], data=A2_vel*SI_to_file.v, dtype='f')
    ...
    grp.create_dataset(Di_hdf5_picle_label['rho'], data=A1_rho*SI_to_file.rho, dtype='f')
